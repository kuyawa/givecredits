...
export async function invoke({ method, args = [], fee = 100, responseType, parseResultXdr, secondsToWait = 10, rpcUrl, networkPassphrase, contractId, wallet, parseMeta=false}) {
  console.log('DATA', JSON.stringify({ method, args, fee, responseType, parseResultXdr, secondsToWait, rpcUrl, networkPassphrase, contractId, wallet, parseMeta }, null, 2))
  ...
  const {raw, txid} = await sendTx(tx, secondsToWait, server)
  console.log('RAW', raw)
  if (responseType === "full") {
